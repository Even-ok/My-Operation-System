...
; 将设置在0:GDTR0处的GDT信息加载到GDTR寄存器中。
		...
		LGDT	[GDTR0]			;禁止内存分页机制(CR0.bit[31]=0), 开启保护模式(CR0.bit[0]=1)。
		MOV		EAX,CR0
...
; 此处根据此头部作相关拷贝。
		...
		ADD		ECX,3			; ECX += 3;
		SHR		ECX,2			; ECX /= 4;
		JZ		skip			; 全局数据在haribote C程序中的偏移地址, 求得haribote C程序中全局数据的起始地址。
		MOV		ESI,[EBX+20]	; 
		ADD		ESI,EBX          
		MOV		EDI,[EBX+12]	; 
		CALL	memcpy
...
GDT0:
    ...
    ; 段内存基址=0, 段长=0xffffffff,dpl=0
		DW		0xffff,0x0000,0x9200,0x00cf	; g32bit
